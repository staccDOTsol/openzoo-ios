...
assert.ok(app.indexOf('OpenZooIde.openSession') !== -1);
...
const chromeReadyAt = app.indexOf("type: 'openzoo-chrome-ready'");
const firstLoadModelsAt = app.indexOf('loadModels();');
assert.ok(chromeReadyAt !== -1);
assert.ok(chromeReadyAt < firstLoadModelsAt);
assert.ok(app.lastIndexOf('renderSidebar();') < chromeReadyAt);
assert.ok(app.lastIndexOf('renderHeader();') < chromeReadyAt);
...
assert.ok(/<body>\s*<div id="oz-boot"/.test(shell));
assert.ok(shell.indexOf('starting') !== -1);
assert.ok(shell.indexOf('z-index: 10000') !== -1);
assert.ok(/background:\s*#0a0a12|#000/.test(shell.split('#oz-boot')[1]));
assert.ok(shell.indexOf("data.type === 'openzoo-chrome-ready'") !== -1);
assert.ok(shell.indexOf('showBoot()') !== -1);
assert.ok(shell.indexOf('setTimeout(hideBoot, 50)') !== -1);
assert.ok(shell.indexOf('setTimeout(hideBoot, 4000)') !== -1);
assert.ok(shell.indexOf('hideBootIfFirstRun') !== -1);
assert.ok(shell.indexOf('DOMContentLoaded') !== -1);
assert.ok(!/navigator\.splashscreen|cordova-plugin-splashscreen/.test(shell));
const launchFn = shell.slice(shell.indexOf('function launchApp()'), shell.indexOf('function showPaywall()'));
assert.ok(launchFn.indexOf('showBoot()') !== -1);
assert.ok(launchFn.indexOf('hideBoot()') === -1);
assert.ok(launchFn.indexOf('iframe.src = GAME_URL') !== -1);
assert.ok(!/openzoo\.fun/.test(launchFn));
